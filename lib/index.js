...
export const useAnimatedRegion = (location = {}) => {
    ...
    const animatedProps = useAnimatedProps(() => {
        ...
        return ({
            coordinate: {
                latitude: (_a = latitude.value) !== null && _a !== void 0 ? _a : 0,
                longitude: (_b = longitude.value) !== null && _b !== void 0 ? _b : 0,
                latitudeDelta: (_c = latitudeDelta.value) !== null && _c !== void 0 ? _c : 0,
                longitudeDelta: (_d = longitudeDelta.value) !== null && _d !== void 0 ? _d : 0,
            },
        });
    });
...
export const useAnimatedClusters = ({ featuresToDisplay, mapDimensions, region, ...options }) => {
    const [points, supercluster] = useClusterer(featuresToDisplay, mapDimensions, region, { ...options });
    const prevPoints = useRef(points);
    const [animatedPoints, setAnimatedPoints] = useState(points);
    useEffect(() => {
        function AnimateMarkers() {
            const prevClusters = prevPoints.current.flatMap((x) => isPointCluster(x) ? x : []);
            ...
            featuresToDisplay.forEach((originalMarker) => {
                const clusterThatWillContainTheOriginalMarker = nextClusters.find((x) => supercluster
                ...
                if (clusterThatWillContainTheOriginalMarker) {
                    ...
                    const prevPointIndex = prevPoints.current.findIndex((x) => x.properties.id === originalMarker.properties.id);
                    if (prevPointIndex >= 0) {
                        prevPoints.current[prevPointIndex].properties.animate = {
                            ...
                            to: {
                                latitude: clusterThatWillContainTheOriginalMarker.geometry
                                    .coordinates[1],
                                longitude: clusterThatWillContainTheOriginalMarker.geometry
                                    .coordinates[0],
                            },
                        };
                    }
                    ...
                    const oldCluster = prevClusters.find((x) => supercluster
                        .getChildren(x.properties.cluster_id)
                        .find((m) => m.id === originalMarker.id));
                    const pointIndex = points.findIndex((x) => x.properties.id === originalMarker.properties.id);
